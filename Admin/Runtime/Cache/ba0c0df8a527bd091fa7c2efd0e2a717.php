                            ...
                            <tr>
                                ...
                                <td>
									<select name="info[type]" >
										...
										<?php if(is_array($menu_list)): foreach($menu_list as $key=>$sk): ?><option value='<?php echo ($sk["id"]); ?>' <?php if($info['type'] == $sk['id']): ?>selected='selected'<?php endif; ?>><?php echo str_repeat('&nbsp;&nbsp;&nbsp;&nbsp;', $sk['level']); if($sk["level"] > 1): ?>|--<?php endif; echo ($sk["menu_name"]); ?></option><?php endforeach; endif; ?>
									</select>
                                </td>
                            </tr>
                            <tr>
                                <th width="100">标题：</th>
                                <td><input id="title" type="text" class="input" size="30" name="info[explain]" value="<?php echo ($info["explain"]); ?>"/></td>
                            </tr>
                            <tr>
                                <th>轮播图片地址：</th>
                                <td>
                                	<div class="instructions del_image">删除</div>
                                    <div class="droparea spot" id="ad_img" style="background-image: url(<?php echo ($info["img_src"]); ?>); cursor:pointer;background-size: 220px 160px;">
	                                    <input type="hidden" name="info[img_src]" id="ad_img_val" value="<?php echo ($info["img_src"]); ?>">
                                    </div>
                            	</td>
                            </tr>
                            <tr>
                                <th>alt属性：</th>
                                <td><input type="text" class="input" size="20" name="info[img_alt]" value="<?php echo ($info["img_alt"]); ?>"/> （seo优化）</td>
                            </tr>
                            <tr>
                                <th>排序：</th>
                                <td><input type="text" class="input" size="5" name="info[sort]" value="<?php echo ($info["sort"]); ?>" /></td>
                            </tr>
                            <tr>
                                <th>跳转地址：</th>
                                <td><input type="text" class="input" size="70" name="info[url]" value="<?php echo ($info["url"]); ?>"/></td>
                            </tr>
