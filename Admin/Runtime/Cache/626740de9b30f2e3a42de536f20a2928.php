	                    ...
	                    <form action="" method="post">
	                        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table1">
	                            <tr>
	                                ...
	                                <td>
										<select id="field_type" name="field_type" style="width:258px" onChange="if(this.value>2 && this.value <7){ $('#set_option').show();$('#zy_php').hide();  } else{ $('#set_option').hide();$('#zy_php').show(); }">
											...
											<?php if(is_array($list_type)): $k = 0; $__LIST__ = $list_type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><option value="<?php echo ($k-1); ?>" <?php if($article_extend_fields_info['field_type'] == ($k - 1)): ?>selected='selected'<?php endif; ?>><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
										</select>					
									</td>
	                            	<td>
	                            ...
	                            <tr>
	                                ...
	                                <td><input name="form_explain" type="text" class="input" size="40" value="<?php echo ($article_extend_fields_info["form_explain"]); ?>" /></td>
	                            	<td><span class='jkd_introduce'>例如:姓名</span></td>
	                            </tr>
	                            <tr>
	                                <th width="120">字段名称：</th>
	                                <td><input name="field_name" id='field_name' type="text" class="input" size="40" value="<?php echo ($article_extend_fields_info["field_name"]); ?>" /></td>
	                            	<td><span class='jkd_introduce'>例如:name（注：英文）</span></td>
	                            </tr>
	                            <tr id="set_option" <?php if($article_extend_fields_info['field_type'] == 3 or $article_extend_fields_info['field_type'] == 4 or $article_extend_fields_info['field_type'] == 5 or $article_extend_fields_info['field_type'] == 6): else: ?>style="display:none;"<?php endif; ?>>
								  <th width="120">预设选项</th>
								  <td width='258'><textarea name="set_option" style="width:258px; height:115px;"><?php echo ($article_extend_fields_info["set_option"]); ?></textarea></td>
	                            ...
	                            <tr>
	                                <th>预设默认值：</th>
	                                <td width="258"><input class="input" name="default_option" type="text" size="40" value="<?php echo ($article_extend_fields_info["default_option"]); ?>" /></td>
	                            	<td><span class='jkd_introduce'>例如:name字段默认值：李刚 默认可以为空</span></td>
	                            </tr>
	                            <tr>
	                                <th>附加CSS：</th>
	                                <td width="258">
	                                	<textarea name="css" cols="40" rows="2" style="height:85px;"><?php echo remove_str_other($article_extend_fields_info['css']);?></textarea>
	                                </td>
	                           	 	<td><span class='jkd_introduce'>例如：style='width:350px; height:30px'class='red'</span></td>
	                            </tr>
	                            <tr>
	                                <th>排序：</th>
	                                <td><input class="input" name="sort" type="text" size="40" value="<?php echo ($article_extend_fields_info["sort"]); ?>" /></td>
	                            	<td></td>
	                            </tr>
	                            <tr>
	                                <th>状态：</th>
	                                <td>
										<select name="is_show" >
											<option value="1" <?php if(!empty($id) and $article_extend_fields_info['is_show'] == 1): ?>selected='selected'<?php endif; ?>>显示</option>
											<option value="0" <?php if(!empty($id) and $article_extend_fields_info['is_show'] == 0): ?>selected='selected'<?php endif; ?>>隐藏</option>
										</select>		
									</td>
	                            	<td></td>
	                            </tr>
