                ...
                <div id="Right">
                    <div class="Item hr">
	                    ...
	                    <div style="width: 100px;float: right;">
	                    	<button type="button" class="btn" onclick="window.location.href='<?php echo U('Jkd/ProductContentModel/product_model_add');?>'">添加字段</button>
	                    </div>
                    </div>
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab">
                        ...
                        <tbody>
                        	<?php if(is_array($product_extend_fields_list)): foreach($product_extend_fields_list as $key=>$jkd): ?><tr>
                        		<td align='center'><?php echo ($jkd["form_explain"]); ?></td>
                        		<td align='center'><?php echo ($jkd["field_name"]); ?></td>
                        		<td align='center'>
                        			<?php if($jkd['field_type'] == 0): ?>单行文本
                        			<?php elseif($jkd['field_type'] == 1): ?>多行文本
                        			<?php elseif($jkd['field_type'] == 2): ?>编辑器
                        			<?php elseif($jkd['field_type'] == 3): ?>下拉列表
                        			<?php elseif($jkd['field_type'] == 4): ?>radio
                        			<?php elseif($jkd['field_type'] == 5): ?>多选列表
                        			<?php elseif($jkd['field_type'] == 6): ?>多选按钮
                        			<?php elseif($jkd['field_type'] == 7): ?>文件上传<?php endif; ?>
								</td>
                        		<td align='center'><input type='text' class='input' name="sort[<?php echo ($jkd["field_id"]); ?>]" value="<?php echo ($jkd["sort"]); ?>" size='1' /></td>
                        		<td align='center'>
                        			<?php if($jkd['is_show'] == 1): ?>显示
                        			<?php else: ?>隐藏<?php endif; ?>
                        		</td>
                        		<td align='center'>
                        			<a href="<?php echo U('Jkd/ProductContentModel/product_model_edit', array('id' => $jkd['field_id']));?>" >编辑</a>&nbsp;&nbsp;|&nbsp;&nbsp;
                        			<a link="<?php echo U('Jkd/ProductContentModel/product_model_del', array('field_name' => $jkd['field_name'],'id' => $jkd['field_id']));?>" href="javascript:void(0)" name="<?php echo ($jkd["form_explain"]); ?>" class="del">删除</a>
                        		</td>
