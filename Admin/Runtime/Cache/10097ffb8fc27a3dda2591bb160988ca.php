                    ...
                    <div class="contentArea">
	                    ...
	                    <div class="main_center">
							<table width="100%" border="0"  align=center cellpadding="3" cellspacing="2" class="table1 table-bordered">
								...
								<?php if(!empty($dirlast)): if($dirpath != './Home/Tpl'): ?><tr bgcolor="#f1f3f5" style="font-weight:bold;">
									<td colspan="5" align="left">
										<img src="__PUBLIC__/tplcenter/file/last.gif"> 
										<a href="__URL__/index/id/<?php echo ($dirlast); ?>" >上级目录</a> 当前目录: <?php echo ($dirpath); ?>
									</td>
								</tr><?php endif; endif; ?>
								<?php if(is_array($list_dir)): $i = 0; $__LIST__ = $list_dir;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(($vo["isDir"]) == "1"): ?><tr>
								 	<td width="30%">
								 		<img src="__PUBLIC__/tplcenter/file/folder.gif" width="16" height="16">
								 		<a href="__URL__/index/id/<?php echo str_replace('.', '' , $vo['pathfile']);?>"> <?php echo ($vo["filename"]); ?></a>
								 	</td>
								    <td width="20%">文件夹</td>
								    ...
								    <td width="20%"><a href="__URL__/index/id/<?php echo str_replace('.', '' , $vo['pathfile']);?>">查看目录</a></td>
								</tr>
								<?php else: ?>
								<tr>
								 	<?php if(in_array(($vo["ext"]), explode(',',"jpg,gif,js,css,html,htm,php"))): ?><td>
								   		<img src="__PUBLIC__/tplcenter/file/<?php echo ($vo["ext"]); ?>.gif" width="16" height="16"> <?php echo ($vo["filename"]); ?>
								   	</td>
								  	 	<?php else: ?>
								   	<td>
								   		<img src="__PUBLIC__/tplcenter/file/other.gif" width="16" height="16"> <?php echo ($vo["filename"]); ?>
								   	</td><?php endif; ?>
								   	...
								   	<td>
								   		<a href="__URL__/Add/id/<?php echo str_replace('.', '', jkd_url_repalce($dirpath, 'desc')); ?>|<?php echo str_replace('.'.$vo['ext'],'*'.$vo['ext'],$vo['filename']);?>">编辑</a> 
								   		<a link="__URL__/Del/id/<?php echo str_replace('.', '', jkd_url_repalce($dirpath, 'desc')); ?>|<?php echo str_replace('.'.$vo['ext'],'*'.$vo['ext'],$vo['filename']);?>" href="javascript:void(0)" class="del" name="<?php echo ($vo["filename"]); ?>">删除</a></td>
								   		<?php }else{ ?>
								   	<td>
								   		<a href="<?php echo ($dirpath); ?>/<?php echo str_replace('.', '' , $vo.filename);?>" target="_blank">浏览</a> 
								   		<a link="__URL__/Del/id/<?php echo str_replace('.', '', jkd_url_repalce($dirpath, 'desc')); ?>|<?php echo str_replace('.'.$vo['ext'],'*'.$vo['ext'],$vo['filename']);?>" href="javascript:void(0)" class="del" name="<?php echo ($vo["filename"]); ?>">删除</a>
								   	</td>
								   	<?php } ?>
