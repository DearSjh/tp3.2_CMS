			...
			<?php elseif(MODULE_NAME == 'Database'): ?>	
                ...
                <div id="Right">
                    <div class="contentArea">
	                    <div class="Item hr">
	                        <div class="current">修改栏目</div>
	                    </div>
	                    <form action="" method="post">
	                        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table1">
	                        	<tr>
	                                <th width="120"><font color='red'>内容模型：</font></th>
	                                <td>
										<select name="content_model">
                                			<option value="article" <?php if($menu_info['content_model'] == 'article'): ?>selected='selected'<?php endif; ?>>普通文章|article</option>
                                			<option value="product" <?php if($menu_info['content_model'] == 'product'): ?>selected='selected'<?php endif; ?>>产品|product</option>
                                			<option value="page" <?php if($menu_info['content_model'] == 'page'): ?>selected='selected'<?php endif; ?>>单页|page</option>
                            			</select>
									</td>
	                            	<td></td>
	                            </tr>
	                            <tr>
	                                <th width="120">语言选择：</th>
	                                <td>
										<select name="lang" style="width: 80px;">
                                			<option value="zh-cn" <?php if($menu_info['lang'] == 'zh-cn'): ?>selected='selected'<?php endif; ?>>简体中文</option>
                                			<option value="zh-en" <?php if($menu_info['lang'] == 'zh-en'): ?>selected='selected'<?php endif; ?>>English</option>
                            			</select>
									</td>
	                            	<td></td>
	                            </tr>
	                            <tr>
	                                <th width="120">是否导航显示：</th>
	                                <td>
										<select name="is_show" style="width: 80px;">
                                			<option value="1" <?php if($menu_info['is_show'] == 1): ?>selected='selected'<?php endif; ?>>显示</option>
                                			<option value="2" <?php if($menu_info['is_show'] == 2): ?>selected='selected'<?php endif; ?>>不显示</option>
                            			</select>
									</td>
	                            	<td></td>
	                            </tr>
	                            <tr>
	                                ...
	                                <td><input name="title" type="text" class="input" size="40" value="<?php echo $menu_info['title'];?>" /></td>
	                            	<td></td>
	                            </tr>
	                            <tr>
	                                <th>keywords：</th>
	                                <td><textarea name="keywords" cols="40" rows="2" style="height:85px;"><?php echo $menu_info['keywords'];?></textarea></td>
	                            ...
	                            <tr>
	                                <th>description：</th>
	                                <td width='258'><textarea name="description" cols="40" rows="5" style="height:125px;"><?php echo $menu_info['description'];?></textarea></td>
	                            	<td><span class='jkd_introduce'><br /><br /><br />(页面描述)<br>优化建议：一般不超过200个字符<br>description用于搜索引擎优化，放在 meta 的 description 标签中</span></td>
	                            </tr>
	                            <tr>
	                                <th width="120">打开方式：</th>
	                                <td>
										<select name="target" id="target">
  											<option value="0" <?php if($menu_info['target'] == 0): ?>selected='selected'<?php endif; ?>>原窗口</option>    
  											<option value="1" <?php if($menu_info['target'] == 1): ?>selected='selected'<?php endif; ?>>新窗口</option>
  										</select>
									</td>
	                            	<td></td>
