...
//组合搜索 start----------------------------->
    	if(!empty($title)){ $where .= " AND article.title LIKE '%" . $title . "%'";}
...
//初始化数据	
    	...
    	$show  = $Page->show();
    	$this->article_list = $db->where($where)->order('id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function article_add(){
    	if(IS_POST){
    		...
    		//扩展字段数据处理
    		$menu_extend_show_list = M('article_extend_show')->join(C('DB_PREFIX').'article_extend_fields on '.C('DB_PREFIX').'article_extend_show.field_id='.C('DB_PREFIX').'article_extend_fields.field_id')->where(C('DB_PREFIX').'article_extend_show.is_show=1 and '.C('DB_PREFIX').'article_extend_show.cid='.$cid)->field(C('DB_PREFIX').'article_extend_fields.*')->order(C('DB_PREFIX').'article_extend_fields.sort')->select();
    		...
    		if ($db->add($data)) {
    			echo json_encode(array('status' => 1, 'info' => "已经发布", 'url' => U('Article/article_list', array('cid' => $data['cid']))));
    		} else {
			...
			//栏目扩展的字段  - select
    		$this->menu_extend_show_list = M('article_extend_show')->join(C('DB_PREFIX').'article_extend_fields on '.C('DB_PREFIX').'article_extend_show.field_id='.C('DB_PREFIX').'article_extend_fields.field_id')->where(C('DB_PREFIX').'article_extend_show.is_show=1 and '.C('DB_PREFIX').'article_extend_show.cid='.$cid)->field(C('DB_PREFIX').'article_extend_fields.*')->order(C('DB_PREFIX').'article_extend_fields.sort')->select();
	...
	public function article_edit(){
		if(IS_POST){
			...
			if ($db->where('id='.$aid)->save($data)) {
				echo json_encode(array('status' => 1, 'info' => "更新成功", 'url' => U('Article/article_list', array('cid' => $cid))));
			} else {
			...
			//读取文章数据
			$this->article_info = M('article')->where('id ='.$aid)->find();
	...
	public function del(){
		...
		if(!empty($id)){
			$last_rs =  M('article')->where('id='.$id)->delete();
			if (!empty($last_rs)) {
